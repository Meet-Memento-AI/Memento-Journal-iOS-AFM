...
#
# MODE (spec 021 R6 sequencing note): supabase-swift (+ two UI packages) are
# still linked today and are in spec 013 R7's deletion manifest. The allowlist
# reflects the TARGET state (RevenueCat only), so this runs REPORT-ONLY by
# default (warns, exits 0). After spec 015's decommission removes the legacy
# deps, flip to enforcing: ALLOWLIST_ENFORCE=1 (must be a hard gate before
# spec 021 closes).
#
