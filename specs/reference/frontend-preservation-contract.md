...
| ID | Behavior | Implementation |
|---|---|---|
| PRES-040 | Empty state: Memento glyph, headline **"Let’s dive deeper into your journal"** (no name greeting), **(amended 2026-08-24)** full-viewport vertical scroller of logo + headline + **horizontally scrolling** suggestion tiles (Figma 709:2320: 160×200 cream card, prompt then circular arrow, no category tag). 16pt gutters and 16pt inter-card gap on the carousel `HStack`; cluster centered when it fits, whole cluster (including logo/headline) scrolls vertically when it overflows. Starters rotated from local `AISuggestionPrompts.json`. | `Views/AI-Chat/AIChatView.swift`, `ChatMessagesView.swift`, `AISuggestionCard` |
