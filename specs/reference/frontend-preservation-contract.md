...
| PRES-041 | Three-state input field: default (history pill + "Chat with Memento" pill + voice pill) / text-active (1–5 line expanding field + send) / voice-active (inline listening panel: cancel/confirm, audio-reactive dots, transcription auto-send). **(amended 2026-09-16, spec [046](../046-photo-capture-and-multimodal-recall.md) R6)** Plus a **photo attachment affordance**, absent from the original contract: an attach button opening a `PhotosPicker` capped at 3 images, and a 112pt thumb row rendered *inside the same glass as the input* with a per-thumb remove button and VoiceOver labels "Attach photo" / "N of 3 photos attached". Photos-only sends are permitted. Library only — no camera in chat. | `Components/AIChat/ChatInputField.swift` (`:106-108`, `:250-265`, `:331-345`, `:443-474`, `:612-637`), `AIChatFooter.swift` |
