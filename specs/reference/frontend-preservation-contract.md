...
| PRES-005 | ~~Floating top header shared across pages~~ **(amended 2026-08-16, spec 027 R3):** each root page now owns its own header — avatar (left) and page-specific actions (right); no center pill slot. The single shared header capped the app at three controls (a fourth shifted the screen ~12pt off-centre). Attached via `safeAreaInset(edge: .top)`, replacing hardcoded header-height arithmetic in three views. | `Components/Navigation/AppHeader.swift`, `Components/Buttons/AvatarInitialButton.swift` |
