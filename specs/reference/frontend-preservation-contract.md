...
| PRES-007 | Floating new-entry FAB on Journal tab, trailing footer slot (`windowBottom + 16`). Labeled glass capsule at that slot: light is black-tinted (`BaseColors.black` at 0.9) with white icon and label; dark flips to white-tinted frost with black icon and label. **"Write your first entry"** when empty (Figma 791:2889), **"New entry"** when the list has entries. | `Components/Buttons/NewEntryFAB.swift`, `Views/Journal/JournalView.swift` |
...
| PRES-021 | Entry cards: date chip over title (Figma 804:3342), 32pt flat fill. Photo entries use the cover as a full-bleed backdrop with tokenized blur/saturation/scrim (804:3343 / 786:2721); excerpt is VoiceOver-only. Tap-to-edit, delete via confirmation dialog ("This action cannot be undone") | `Cards/JournalCard.swift` |
| PRES-022 | Month-picker sheet (wheel month+year) synced to scroll position | `Views/Journal/JournalView.swift` |
| PRES-023 | Notion-style full-page editor: zooming navigation page (not a sheet — no drag handle), back, date pill, title field ("Give your entry a title..."), spacious body editor ("Start writing your journal..."), submit arrow disabled when body empty, keyboard-aware padding. Opens/closes with the native SwiftUI zoom transition from the new-entry FAB or journal card. **(amended 2026-08-25, zoom backdrop):** the morph runs over the live root page — the journal timeline at its current scroll position, or Chat when opened from the summarize control — and never over a blank or system-colored plate. ContentView's overlay stack is layered above `RootPager` for exactly this reason, so its navigation container must stay transparent (`containerBackground(.clear, for: .navigation)` *inside* the stack plus `transparentNavigationContainer()`), and every route pushed on it must paint its own opaque fill. **(amended 2026-09-05, photo editor):** when a cover is attached, the photo is a full-bleed treated backdrop at blur 100 (Figma 818:4006), white title and 60% body; empty placeholders as above; no-photo editor uses the same type stack on `theme.background`. **(amended 2026-09-09, editor paging):** an `.edit(UUID)` route mounts `EntryEditorPager`, so a horizontal thumb swipe walks the timeline's own order (newest left, older right) without returning to the list; `.create` routes have no neighbours and still mount `AddEntryView` directly. Built on `TabView` + `.page` for the reasons PRES-004 gives, and it carries the same commit haptic, so the editor swipe and the root swipe are one gesture vocabulary. Two constraints the implementation must keep: the zoom source stays the card that was *tapped* (`YourEntriesView`'s month groups are a `LazyVStack`, so a card paged to across an unbuilt month has no live `matchedTransitionSource` to morph back into), and paging away **commits** rather than discards — a swipe is navigation, so the leaving page folds in any live dictation and writes the entry if it is dirty, unlike Back. **(amended 2026-09-09, memory reveal; 2026-09-10, header lens):** "View memory" (Figma 898:1784 treated / 824:4211 revealed) is not a second screen — the editor's own backdrop dissolves its treatment away over ~0.85s on a decelerating curve, driven by the same `shaderRevealProgress` scalar a capture animates. The toggle is a header lens control (`LensBlur` while treated, `Lens` while revealed) that stays mounted and swaps the glyph; the footer no longer carries View memory / View entry, viewing an existing entry has no FABs, and compose/edit keep Capture (and compose mic) even with a cover. Three constraints, each of them a defect the earlier full-screen overlay had: the header-left control is `chevron.left` closing the *editor* in every state, because a revealed memory must not swap in an `xmark` that means something narrower in the one spot the user has learned Back lives; nothing may mount or unmount across the toggle, or the glass tears down mid-dissolve and flashes, so only the entry's own surfaces (title, body, compose mic) fade and `accessibilityHidden` belongs on the scroller rather than on the stack holding the chevron (back, lens, and pencil/check stay visible on a revealed cover); and the always-mounted sharp layer is the list thumbnail the treated bitmap is rendered from, with the full-resolution cover mounted only while `shaderRevealProgress < 1`. Editor glass buttons stay on `Glass.native()` frost in every editor state (treated cover, revealed cover, and canvas) — a photo-derived wash on those pills did not read well. `chromeTint` remains available for other photo chrome, but AddEntry buttons do not apply it. **(amended 2026-09-10, device freeze):** the backdrop layer carries no animated transform and no per-frame glass rebuild — `JournalBackdropRenderer`'s header is the reason: this is a screen-sized layer with five Liquid Glass surfaces above it, and a live filter, an animated `scaleEffect`, or a 1600px sharp source on that layer all stall the editor on device while looking fine in the Simulator. Pager neighbours load the already-decoded thumbnail on mount and wait until `editorPageIsActive` to decrypt the full file or run CoreImage. | `Views/Journal/AddEntryView.swift`, `EntryEditorPager.swift`, `EntryEditorDestination.swift`, `ContentView.swift`, `Utilities/TransparentNavigationContainer.swift`, `Utilities/JournalBackdropContrast.swift`, `Resources/JournalBackdropShader.swift` |
| PRES-024 | Editor voice-dictation FAB: expands to live duration display, red stop state, mic-permission and recording-failed alerts. **(amended 2026-09-09, dictation toggle.)** Silhouette unchanged; the control moved out of `AddEntryView` into `EditorDictationPill` and gained two transient states, `starting` and `transcribing`, both drawn as the `ProgressView` the save control already uses. Three reasons, each a live defect: the capsule carried `.glassEffect` with **no `.contentShape`**, so only the glyph and the timer text were tappable and a stop tap anywhere else on the pill did nothing — the mic could be started but not stopped; the pill derived its state from `speechService.isRecording`, which only flips true after permission, asset and analyzer bring-up, so it read idle through the whole start and a second tap raced a second engine; and observing `SpeechService` at the editor root re-rendered the backdrop, all four glass surfaces and the body editor on each of the service's ~50 publishes/sec while recording. Saving mid-dictation also dropped the transcript — the body was updated after the saved text had been captured. | `Components/Journal/EditorDictationPill.swift`, `AddEntryView.swift` + speech service (2.0: `SpeechAnalyzer`, spec 018) |
...
| PRES-043 | Response action bar: copy, thumbs up (persisted positive), thumbs down (persisted negative + reason sheet), regenerate, overflow → Report answer | `AIOutputComponent.swift` (spec 041: `AnswerFeedbackStore`; spec 042: optional verification upload; 2.0 eval feed remains spec 022) |
...
| PRES-092 | Liquid Glass surfaces render as one system. **(re-adopted 2026-08-16, superseding the 2026-08-07 removal.)** The removal was made against Simulator rendering — where glass is expected to read flat gray — and spec 024's device pass (its Task 7) was never completed. Two concrete causes were found and fixed: an **opaque `ScrollEdgeFade(.top)` scrim** painted directly behind the floating header, leaving glass nothing to refract; and **adjacent glass with no shared sampling region** (glass cannot sample glass), which read as stacked/double glass. Rules now in force: one `GlassEffectContainer` per adjacent cluster, spaced to match the layout; `.glassEffect` applied to the view **containing** the content, never as a sibling `.background(...)`, or the content gets no vibrancy treatment; never an opaque fill or scrim beneath glass; `.regular` only; content-layer surfaces (list rows, cards, the crisis card) stay flat. Theme-backed chrome stays **untinted** so the system adapts. Photo-backed chrome may carry a backdrop-derived wash (`JournalBackdropContrast.chromeTint`, capped at `chromeTintCeiling`) for glyph legibility, not prominence. **One** low-opacity prominence tint (`Glass.prominentFrost`) is allowed on Capture so the primary action still reads through frost. `theme.glassBorder` remains a non-glass insight-card stroke and is **not** a glass API. **(2026-08-16)** `ScrollEdgeFade` — the opaque scrim named above — is now deleted outright rather than merely unused at the top edge; see PRES-095. **(amended 2026-09-10 — adaptive editor chrome.)** **(re-verified 2026-09-24, spec 052 R6):** the iPad column cap narrows the header row's frame *inside* the existing single `GlassEffectContainer`. No fill is added, no cluster split, no second container — the rules above are unchanged. The Simulator renders glass flat, so the one-cluster check stays a device check. | `Components/Navigation/AppHeader.swift`, `Components/Buttons/{AvatarInitialButton,NewEntryFAB}.swift`, `Components/AIChat/ChatInputField.swift`, `Views/Journal/AddEntryView.swift`; scroll edges in `YourEntriesView.swift` / `AIChatView.swift`; tokens in `Resources/Theme.swift` / `Resources/JournalBackdropShader.swift` |
