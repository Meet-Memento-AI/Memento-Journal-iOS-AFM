...
> | R6's support-URL and support-email decisions | `docs/app-store/00` B1/D3 — **the support email is now decided: `hello@withmemento.ai`** (standardized on `contact@sebastianmendo.design` 2026-08-11, re-pointed to the product domain 2026-09-17) |
...
>   (`sebmendo1/withMemento` @ `Memento-v1.1`). See `docs/app-store/11` §1.
...
|---|---------|----------|----------|
| 1 | `ITSAppUsesNonExemptEncryption` not set anywhere → encryption-compliance prompt/warning on every ASC upload. App uses HTTPS/standard crypto only (PBKDF2 via CommonCrypto, ATS-only networking), so `false` is the correct declaration. | `withMemento/Info.plist` (absent); `grep -r ITSApp` → empty | BLOCKER |
| 2 | App icons are **JPG**: light = `AppIcon.jpg`, tinted = `AppIcon 1.jpg` (byte-identical copy of light, not actually tinted), and the declared **dark slot has no file**. Apple expects PNG without alpha; JPG risks upload warnings, and the empty dark declaration is untidy. | `withMemento/Assets.xcassets/AppIcon.appiconset/Contents.json` + `.jpg` files | HIGH |
| 3 | `AccentColor` is pure white (1,1,1,1) — invisible tint on light backgrounds. | `withMemento/Assets.xcassets/AccentColor.colorset/Contents.json` | MEDIUM |
| 4 | Extra targets in the project: `Sign In with Apple` (Apple sample-code target, bundle id `com.sebmendo.withMementoAI.Sign-In-with-Apple`) and `UIPlayground`/`UIPlaygroundTests`/`UIPlaygroundUITests` (bundle ids under `com.testing.*`). Submission noise; must never enter the archive. | `withMemento.xcodeproj/project.pbxproj` targets list | HIGH |
| 5 | Microphone/speech usage strings defined **twice with different wording**: pbxproj `INFOPLIST_KEY_NS{Microphone,SpeechRecognition}UsageDescription` ("…transcribe your journal entries using voice") vs `Info.plist` ("…transcribe your voice into journal entries"). `GENERATE_INFOPLIST_FILE = YES` merges both — winner unpredictable. | `project.pbxproj:908-909,944-945` vs `withMemento/Info.plist:44-47` | MEDIUM |
| 6 | ~~ASC listing needs a **Support URL** — only a support email is wired in-app (`support@sebastianmendo.com`, which differs from the dev account email; mailbox unverified).~~ **RESOLVED** — see checklist D2/D3; the address is now `hello@withmemento.ai`. A ready support page exists at repo root (`support.html`) but is not hosted at a known URL. | `AboutSettingsView.swift:274`, `DataUsageInfoView.swift:242`, root `support.html` | MEDIUM |

| 7 | **(found during implementation, 2026-07-13)** The Release app bundle shipped junk via the `withMemento/` synchronized group: 5 SQL migration files (from a previously unknown **third** migrations dir nested at `withMemento/supabase/` — see spec 003), `Debug/Release.xcconfig` + templates, `config.toml`, `cli-latest`, `PLAN.md`, `File.txt`, `Configuration.storekit`, `PreviewMocks.json`, dev docs (`COMPONENT_TEMPLATE.md`, `UTILITIES_GUIDE.md`, `MonthlyInsightCard+Usage.md`, `Fonts/README.md`), a loose `Google-Icon.svg`, and `SupabaseConfig.swift.template`. Shipping DB schema + internal docs in the binary is information disclosure and submission noise. | verified via Release bundle listing | HIGH |
| 8 | **(found during implementation)** All four xcconfig files had the `//`-comment trap: `SUPABASE_URL = https://…` truncates to `https:` because xcconfig treats `//` as a comment — so following the documented local-xcconfig setup produced a silently broken backend URL. | `withMemento/Config/*` | HIGH (latent) |
...
**Acceptance:** the `withMemento` scheme's archive action builds only the app target;
...
**Acceptance:** `support.html` hosted (e.g. `https://sebmendo1.github.io/withMemento/support.html`
...
- [x] 1. Add `ITSAppUsesNonExemptEncryption` = `false` to `withMemento/Info.plist`. (R1)
...
- [x] 4. All four sample targets removed via `xcodeproj` gem (targets, build phases,
      ...
      only withMemento + its two test targets. (R4)
...
- [x] 9. **(added during implementation)** Bundle hygiene: stray `File.txt` deleted;
      nested `withMemento/supabase/` moved to `supabase/UNRECONCILED-app-folder-copy/`
...
| Field | Value | Status |
|-------|-------|--------|
| Bundle ID | `com.sebmendo.withMementoAI` | in project ✅ |
| Display name | Memento | in project ✅ |
| Category | Lifestyle (`public.app-category.lifestyle`) | in project ✅ |
| Privacy Policy URL | `https://sebmendo1.github.io/withMemento/privacy.html` | live, linked in-app ✅ |
| Support URL | `https://sebmendo1.github.io/withMemento/support.html` | file hosted this spec — **confirm it renders after push** |
| Marketing URL (optional) | `https://sebmendo1.github.io/withMemento/` | live ✅ |
| Export compliance | `ITSAppUsesNonExemptEncryption=false` in plist — answer "None of the above / standard encryption" if asked | ✅ |
| Support email | ✅ **DECIDED** — `hello@withmemento.ai` (checklist D3). Historical note: three addresses were in circulation when this row was written — in-app `support@sebastianmendo.com`, support page `support@meetmemento.app`, account `contact@sebastianmendo.design`. All three are retired. Send/receive test on the new address is still ☐ user. | ✅ agent / ☐ user |
...
- [x] `grep -c "com.testing" project.pbxproj` → 0; `xcodebuild -list` shows only
      withMemento/withMementoTests/withMementoUITests; built product has no
      `PlugIns/`. ✅
...
- *(2026-07-23: this guard is superseded by spec 023, which removes the SIWA
  ...
  Sign in with Apple **entitlement** (`withMemento/withMemento.entitlements`) is unrelated
