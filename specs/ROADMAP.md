...
| 10 — Model tier (added 2026-09-25) | [051](051-on-device-model-tier.md) | Every generation records which on-device model ran (AFM 3 Core Advanced, AFM 3 Core, or pre-AFM 3); Core Advanced is the model on every device that can run it, Core only elsewhere; Core's budgets unchanged | draft — plan only. R0 (read the Xcode 27 GM `FoundationModels.swiftinterface` for a tier selector) needs a Mac and decides explicit selection vs OS-chosen + inferred tier. Pure-Swift tier resolver, provenance, and perf-line work can land first |
