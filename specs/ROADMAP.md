...
| 8 — Harness depth (added 2026-09-06) | [044](044-agentic-harness-depth.md) | Retrieval measured offline and fit to gold (`RetrievalGate` recall@5 ≥ 0.85); notebook turns can take a second, bounded search hop; `ask-core@16` prefill ≤ 55% of `ask@15`; the lens learns from the journal only with consent | draft — four independently shippable phases: 1 Evidence (passage index, gate, theme priors — iOS 26 SDK), 2 Prompt (core + channel suffixes, exemplar/schema A/B), 3 Agency (`SearchJournalTool`, token budgeting — iOS 27 SDK), 4 Profile (living lens + Settings consent). Computed insights / `PeriodReflection` to be minted as 045 |
