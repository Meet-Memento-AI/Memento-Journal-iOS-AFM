...
enum SupertonicBridge {
    ...
    static func toFloat32(_ arr: MLMultiArray) -> [Float] {
        ...
        case .int32:
            ...
            for i in 0..<n { out[i] = Float(p[i]) }
        case .int8:
            // FORK DIVERGENCE 5 — see VENDORING.md.
            // `MLMultiArrayDataType.int8` was added in iOS 26, after this code was
            // written upstream, so it fell through to `@unknown default: break` and
            // this function returned an ALL-ZERO array: silent audio, no error, no
            // crash. That is the worst failure mode a TTS engine has, and it matters
            // here because we ship an 8-bit palettized VectorEstimator.
            let p = arr.dataPointer.bindMemory(to: Int8.self, capacity: n)
            for i in 0..<n { out[i] = Float(p[i]) }
        @unknown default:
            // A genuinely unknown future case. Returning zeros silently is how the
            // int8 bug hid, so make it loud in debug rather than merely quiet.
            assertionFailure("unhandled MLMultiArrayDataType: \(arr.dataType.rawValue)")
        }
        return out
    }
