...
|---|---|---|---|
| **A0.1** | Publish `docs/{privacy,terms,support,index}.html` from **this** repo (enable GitHub Pages → `/docs`). Point ASC Privacy + Support URLs at the new host. | ☐ user + agent prep | ✅ **Pages half closed** (`00` A6/A6a). `curl` → **200** for all four. Acceptance corrected 2026-09-24: the prohibition is `openai\|gemini\|google` — **not `supabase`**. Spec 042 *deliberately* discloses the opt-in feedback egress, and `check_live_legal_urls.sh` was fixed for exactly this on 2026-09-12; this criterion had not been. Remaining: the ASC URL paste (D1/D2) |
...
| **A0.3** | Set the ASC App Privacy nutrition label to the **spec-042 target**: Tracking = **No**; declare **Other User Content**, **Other Data Types**, and **User ID** — *linked, not tracking* — with purposes **App Functionality + Analytics**, for the opt-in Share Quality Feedback path. Journal content is **not** collected; the CloudKit private DB is the user's iCloud, not our collection. | ☐ user | Label ↔ `PrivacyInfo.xcprivacy` ↔ published policy identical. **Corrected 2026-09-24:** this row previously said **"Data Not Collected"**, which is the label that caused the **November 2025 rejection** and directly contradicts `00` D6, `00` B3's spec-042 amendment, and `docs/app-store/README.md`. `PrivacyInfo.xcprivacy`'s collected types are no longer empty. A session following the old wording would have re-created the rejection |
