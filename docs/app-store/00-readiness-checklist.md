...
| A6a | **Publish the `docs/` changes.** A6 enabled Pages on the right repo, but Pages serves what is *committed to `main`*, so an edited-but-uncommitted `docs/` is invisible in production. This is the November 2025 root cause in a new costume: the fix exists in the repo and not on the web. | agent + ☐ user | Gate S | 🔴 **Open 2026-09-17** — `docs/{privacy,terms,support,index}.html` carry `hello@withmemento.ai`; the live pages still serve `contact@sebastianmendo.design`. `scripts/ci/check_live_legal_urls.sh` fails on the byte-comparison until this is pushed. **Verify with the gate, not with `git log`** |
...
> **Already fixed, do not redo (required-reason APIs):** the stale EmailAddress /
> Name tracking-era entries are gone. **Amended 2026-09-11 (spec 042):**
> `NSPrivacyCollectedDataTypes` is no longer empty — it declares Other User
...
| C3a | **The build Mac cannot currently build the app at all.** `xcodebuild -showdestinations` returns **zero available destinations**: `/Applications/Xcode.app` is 26.6 (17F113) shipping the **iOS 26.5 SDK**, but the **iOS 26.5 platform component is not installed**, so both `Any iOS Device` and the physical iPhone report *"iOS 26.5 is not installed"*. The installed simulator runtimes (26.0, 27.0) are not offered to this Xcode either. The only toolchain that can build is `~/Downloads/Xcode-beta.app` (**Xcode 27.0 beta, 27A5228h**) — and **Apple does not accept App Store builds made with beta software**, so it cannot produce the submission. | ☐ user | 🔴 **Open, found 2026-09-17.** Fix: Xcode → Settings → Components → download the **iOS 26.5** platform. Then re-verify with `xcodebuild -showdestinations` before archiving. **This blocks the build-4 decision** (C1) and therefore this entire submission — nothing else in Section C can be evidenced until it is cleared |
...
> `MeetMemento/Resources/Constants.swift` hard-codes that base URL, so moving the
...
| D7 | App Review Information: contact name/email/phone, notes, attachments. No demo account needed (no login) — but the notes must **say so**. | ☐ user | 🔴 **Reopened 2026-09-17 — this is what the September 2026 Guideline 2.1 rejection cited.** The notes *were* pasted; they did not answer Apple's six-item information request (purpose/audience, regional differences, and protected-material licensing had no answer anywhere), and **nothing was attached**. Restructure `metadata/en-US/review_notes.txt` per `08` §2, then paste. See `11` §1 |
| D7a | **A demo video recorded on a physical device is attached** to App Review Information. Apple asked for it by name and the app was rejected without it. | agent + ☐ user | ☐ Open — shot list in `08` §4. Physical Apple Intelligence iPhone, latest iOS, Release build, **must begin at app launch**. Store the file outside this public repo |
...
Seven gates enforce the machine-checkable half of this library continuously, so
it cannot silently rot between releases. Each was verified to **fail on a
planted violation** before being wired.

**A gate nothing runs is not a gate.** `check_live_legal_urls.sh` sat in
`scripts/ci/` unwired from August to September 2026 and drifted into failing on
*correct* content — it forbade any mention of Supabase while spec 042
deliberately added a Supabase disclosure. A gate that cries wolf gets ignored,
which is the same failure mode that let the support-URL 404 reach a rejection.
When adding a script here, wire it in the same commit.
...
| `scripts/ci/check_app_size.sh` | Release bundle stays under the recorded ceiling (200 MB, the cellular-download threshold) | Blocking, in `ios-build-online.yml` — **new 2026-09-12** (C10) |
