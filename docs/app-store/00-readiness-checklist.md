...
**For the submission sitting itself, use
[`14-submission-packet.md`](14-submission-packet.md)** — every field with its source
file, the Free determination and its evidence, and a look-and-paste list for the six
...
| B4 | **The published privacy policy will become an overclaim when Z1 ships.** `docs/privacy.html` (the corrected version, not yet published) says content is *"processed on the device and are not sent to us or to any third-party AI service"* and makes no mention of **Private Cloud Compute**. That is accurate today, because no Z1 routing has shipped — and becomes false the moment `specs/017`'s PCC path lands. `REQ-POS-001` governs app strings and store copy; the privacy policy is the one place making the same claim *legally*. | agent | **5.1.2(i)**, `REQ-POS-001` | 🔵 **Deferred by design — do not action for 1.x (annotated 2026-09-24).** This row is not work waiting to be done; it is a precondition on `specs/017`. The current wording is *accurate today* because no Z1 routing exists in the binary, and submit-gate item 2 says in terms: "Do **not** mention Private Cloud Compute until Z1 ships." Writing the Z0/Z1 text now would itself be a 2.3 / 5.1.2(i) accuracy defect — the same class of error as the stale policy, pointed the other way. **Trigger:** the first commit that routes generation to PCC. **Target content:** `01` §5.1.1(i). **Blocking:** recorded as a precondition inside `specs/017`. What *was* actionable — PCC language that had leaked into 1.x-facing docs (`04` permitted line, `11` §5.2 Resolution Center reply, `03` label table) — is fixed as of 2026-09-24. |
| B5 | **A third, older privacy policy is what users actually see.** `PRIVACY_POLICY.md` (repo root) describes Gemini and Supabase; the **live** page describes OpenAI, Google, and Supabase; `docs/privacy.html` is correct but unpublished. Three versions, one of them served. | agent + ☐ user | **5.1.1(i)** | ✅ **Closed 2026-08-17** — in-repo `docs/*.html` is canonical; Pages now serves this repo. **Fully closed 2026-09-24:** the third version is *gone*, not merely demoted — `PRIVACY_POLICY.md` was deleted in `01f0060`, verified on the filesystem. Until then it still sat at the repo root with its "Google Gemini 2.5 Flash" section, one `cat` away from being quoted as authoritative. Remaining: ASC URLs must match (D1/D2). |
